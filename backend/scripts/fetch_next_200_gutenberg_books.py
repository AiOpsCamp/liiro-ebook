#!/usr/bin/env python3
"""
Fetch Next 200 Most Famous & Downloaded Books from Project Gutenberg
...
TARGET_JSON = os.path.join(DOCS_DIR, "NEXT_200_GUTENBERG_BOOKS.json")
TARGET_MD = os.path.join(DOCS_DIR, "NEXT_200_GUTENBERG_BOOKS_CATALOG.md")
...
def load_all_existing():
    # 1. TOP 500 & Missing
    for p in ["TOP_500_GUTENBERG_BOOKS.json", "missing_top_gutenberg_books.json", "TOP_100_GUTENBERG_BOOKS.json"]:
...
EXCLUDED_PATTERNS = [
    ...
    r"the works of .* volume", r"selected works", r"various"
]
...
def scrape_next_200():
    ...
    while len(new_books) < max_needed and page <= 60:
...
def generate_markdown(books):
    lines = [
        "# 🏛️ Top 200 Next Most Popular & Downloaded Project Gutenberg Masterworks",
        "",
        "> **Catalog Scope**: 200 Highly-Demanded Public Domain Masterworks  ",
        "> **Exclusivity**: Verified 100% Brand-New (NOT present in Liiro Ebook DB, Standard Ebooks, or any previous catalog)  ",
    ...
    lines.append("python3 -u backend/scripts/run_gutenberg_batch_importer.py --catalog=backend/docs/NEXT_200_GUTENBERG_BOOKS.json")
    lines.append("```")
