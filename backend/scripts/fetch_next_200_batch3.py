#!/usr/bin/env python3
"""
Fetch Next 200 Most Famous & Downloaded Books from Project Gutenberg (Batch 3: Books 201-400)
that are NOT in any of our existing lists or databases yet.
"""
...
TARGET_JSON = os.path.join(DOCS_DIR, "NEXT_200_GUTENBERG_BOOKS_PART2.json")
TARGET_MD = os.path.join(DOCS_DIR, "NEXT_200_GUTENBERG_BOOKS_PART2_CATALOG.md")
...
def load_all_existing():
    # 1. Past catalogs including first batch of 200
    for p in ["TOP_500_GUTENBERG_BOOKS.json", "missing_top_gutenberg_books.json", "TOP_100_GUTENBERG_BOOKS.json", "NEXT_200_GUTENBERG_BOOKS.json"]:
...
# Ignored non-literary terms (encyclopedias, periodicals, dictionaries, raw lists)
EXCLUDED_PATTERNS = [
...
def scrape_next_200():
    ...
    print(f"\n🚀 Beginning Search for Next {max_needed} Popular Gutenberg Masterworks (Batch 3)...")

    while len(new_books) < max_needed and page <= 100:
...
def generate_markdown(books):
    lines = [
        "# 🏛️ Top 200 Next Most Popular & Downloaded Project Gutenberg Masterworks (Part 2)",
        "",
        "> **Catalog Scope**: 200 Highly-Demanded Public Domain Masterworks (Batch 3: Books 201-400)  ",
        "> **Exclusivity**: Verified 100% Brand-New (NOT present in Liiro Ebook DB, Standard Ebooks, Part 1, or any previous catalog)  ",
    ...
    lines.append("To import all 200 of these newly cataloged books into Liiro Ebook:")
    lines.append("```bash")
    lines.append("cd /Users/humayunrashid/multicamp/liiro-ebook")
    lines.append("python3 -u backend/scripts/run_gutenberg_batch_importer.py --catalog=backend/docs/NEXT_200_GUTENBERG_BOOKS_PART2.json")
    lines.append("```")
