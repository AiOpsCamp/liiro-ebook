...
def generate_audio_for_text(kokoro, full_text, voice_id):
    # Process first ~2,500 chars (approx 3-4 mins full audio per voice per chapter for super rich demo)
    text_segment = full_text[:2500]
...
def generate_full_chapters_audio():
    ...
    print(f"📖 Found {len(chapters)} chapters for '{story_title}'")

    print("🎙️ Initializing Kokoro v1.0 Engine...")
    ...
    for ch in chapters:
        ...
        print(f"\n🎧 [Chapter {ch_num}] {raw_title} (Generating ~3-4 minutes audio for 4 voices)...")
        ...
        for v in VOICES:
            ...
            if samples is not None:
                ...
                print(f"   ✅ {v['name']}: voice_{v['key']}_chapter_{ch_num}.mp3 ({dur_sec/60:.1f} mins / {int(dur_sec)}s)")

    print("\n==================================================")
    print("🎉 MULTI-VOICE FULL AUDIO GENERATION COMPLETE!")
    print("==================================================")

if __name__ == "__main__":
    generate_full_chapters_audio()
